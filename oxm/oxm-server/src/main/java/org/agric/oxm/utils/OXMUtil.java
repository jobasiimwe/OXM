...
public final class OXMUtil {
	 ...
	 * (OXMConstants.OPERATING_SYSTEM_LINUX.equalsIgnoreCase
	 * (OXMConstants.OPERATING_SYSTEM) || OXMConstants.OPERATING_SYSTEM_FREEBSD
	 ...
	 * ".oxfam_files"; } else { filepath = System.getProperty("user.home") +
	 * File.separator + "Application Data" + File.separator + "oxfam_files"; }
	 * 
	...
	public static String createUsername(String first, String last) {
		...
		if (last.indexOf(' ') > 0) {
			username = last.substring(0, last.indexOf(' '));
		} else {
			username = last;
		}

		if (first.indexOf(' ') > 0) {
			username += "." + first.substring(0, first.indexOf(' '));
		} else {
			username += "." + first;
		}
		return username;
