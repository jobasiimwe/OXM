...
<%@ attribute type="org.agric.oxm.model.Season" name="season" rtexprvalue="true" required="true"%>
...
<tr id="${season.id }">
	<td><c:if test="${printCheckBox }">
			<sysTags:rowcheckbox nameOfItemOnPage="${nameOfItemOnPage}" id="${season.id }" />
		</c:if> ${status.count }</td>
	<td>${season.name }</td>
	<td style="display: none;"><sysTags:print-date date="${season.startDate}" /></td>
	<td style="display: none;"><sysTags:print-date date="${season.endDate}" /></td>
