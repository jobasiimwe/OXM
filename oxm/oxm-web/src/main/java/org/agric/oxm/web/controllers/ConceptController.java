...
public class ConceptController {
	...
	public ModelAndView viewConceptHandler(ModelMap model,
			@PathVariable("cid") String conceptCategoryId)
	...
	public ModelAndView addConceptHandler(ModelMap model)
			throws SessionExpiredException {
		prepareConceptFormModel(model);
		WebConstants.loadLoggedInUserProfile(OXMSecurityUtil.getLoggedInUser(),
				model);
	...
	public void deleteConceptHandler(
			@RequestParam("selectedConcepts") List<String> ids,
	...
	public ModelAndView searchConcept(
		...
		List<Concept> concepts = conceptService
				.getConceptsWithParams(params, 0);
	...
	public ModelAndView viewConceptCategoryHandler(ModelMap model)
			throws SessionExpiredException {
		List<ConceptCategory> conceptCategories = conceptService
				.getConceptCategories();
		model.put("conceptcategories", conceptCategories);
