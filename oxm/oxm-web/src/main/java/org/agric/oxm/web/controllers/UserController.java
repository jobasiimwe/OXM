...
public class UserController {
    ...
    public ModelAndView saveUserHandler(@PathVariable("qpage") String qPage,
	    @ModelAttribute("user") User user, @RequestParam(value = "userPic",
		    required = false) MultipartFile userPic, ModelMap model) throws IOException, SessionExpiredException {

	User existingUser = user;
	try {
	    if (!WebConstants.isFileAnImage(userPic)) {
		throw new Exception("error: the file uploaded for the user photo is not an image.");
	    }

	    if (!WebUtils.isValidSize(userPic, WebConstants.DEFAULT_IMAGE_SIZE_IN_BYTES)) {
	...
	if (qPage.equals("0")) {
	    WebConstants.loadLoggedInUserProfile(OXMSecurityUtil.getLoggedInUser(), model);
	    model.put(WebConstants.MODEL_ATTRIBUTE_SYSTEM_MESSAGE, "User saved sucessfully.");
	}
	...
	if (qPage.equals(":")) {
		...
		if (loginedUser.hasAdministrativePrivileges()) {
		    ...
		    model.put(WebConstants.MODEL_ATTRIBUTE_ERROR_MESSAGE, "Invalid path");
		}
	...
	if (user != null) {
		...
		WebConstants.writePictureOnResponse(contentType, user.getProfilePicture(),
			response);
	    }
	}
    }
	...
	if(user != null){
		...
		model.put(WebConstants.MODEL_ATTRIBUTE_SYSTEM_MESSAGE, "User deleted sucessful");
